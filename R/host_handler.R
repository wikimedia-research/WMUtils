...
#'@details \code{host_handler} extracts the host from a URL and, if asked, also strictly filters the resulting hosts,
#'extracting general organisational handles (for example "Google", or "Reddit").
#'
#'@param urls a vector of URLs
#'
#'@param strict whether or not to extract org handles; set to TRUE by default. This is mostly useful for referer
#'parsing.
#'
...
#'
#'@export
host_handler <- function(urls, strict = TRUE){
  
  #Iconv
  urls <- iconv(urls, to = "UTF-8")
  
  #Extract domain
  urls <- unlist(lapply(urls,function(x){
